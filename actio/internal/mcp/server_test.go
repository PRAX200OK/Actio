...
func TestListResources_WithActFiles(t *testing.T) {
	...
	for _, f := range []string{"index.yaml", "architecture/system.md", "rules/rules.md", "tasks/task.md"} {
		_ = os.WriteFile(filepath.Join(actRoot, f), []byte("x"), 0o644)
	}

	res := listResources(dir)
	if len(res) < 4 {
		t.Errorf("expected at least 4 resources, got %d", len(res))
	}
	const prefix = "actio://"
	for _, r := range res {
		uri, _ := r["uri"].(string)
...
func TestReadResource_Success(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "actio", "index.yaml")
...
func TestHandleRequest_ListResources(t *testing.T) {
	...
	_ = os.WriteFile(filepath.Join(actRoot, "index.yaml"), []byte("x"), 0o644)
...
func TestHandleRequest_ReadResource(t *testing.T) {
	...
	params, _ := json.Marshal(map[string]string{"uri": "actio://actio/index.yaml"})
